...
use unsafe_review_core::{ReviewCard, SafetyObligation, WitnessRoute};

use super::super::TRUST_BOUNDARY;

pub(super) fn diagnostic_data(card: &ReviewCard) -> Value {
    json!({
        "card_id": &card.id.0,
        "operation": &card.operation.expression,
        "operation_family": card.operation.family.as_str(),
        "hazards": card.hazards.iter().map(|hazard| hazard.as_str()).collect::<Vec<_>>(),
        "required_safety_conditions": card.obligations.iter().map(obligation_json).collect::<Vec<_>>(),
        "evidence_summary": evidence_summary(card),
        ...
        "witness_routes": card.routes.iter().map(witness_route_json).collect::<Vec<_>>(),
        "verify_commands": &card.next_action.verify_commands,
        "trust_boundary": TRUST_BOUNDARY
    })
}

fn obligation_json(obligation: &SafetyObligation) -> Value {
    json!({
        "key": &obligation.key,
...
fn evidence_summary(card: &ReviewCard) -> Value {
    json!({
        "contract": {
...
fn witness_route_json(route: &WitnessRoute) -> Value {
    json!({
