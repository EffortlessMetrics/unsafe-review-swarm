...
pub(super) fn build_card(
        ...
        == OperationFamily::StableByteSourceGetterReentry
    {
        ...
        "Review this stable-byte-source-native-ffi-read card through an observable-red-green proof path: add or expose guard evidence for a disjointness or copy boundary, confirm the safe JS caller route through manual review, keep input/output byte spans stable before native handoff, and keep the PR inside that native-FFI aperture.".to_string()
...
fn operation_contract_override(family: &OperationFamily) -> Option<ContractEvidence> {
    match family {
        ...
        OperationFamily::StableByteSourceNativeFfiRead => Some(ContractEvidence::present(
            "Stable-byte-source native FFI read heuristic uses byte-stability evidence, not unsafe API safety docs",
        )),
        _ => None,
    }
}
