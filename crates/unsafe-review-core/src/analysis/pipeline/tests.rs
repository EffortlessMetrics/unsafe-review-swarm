use super::identity::unsafe_call_path;
use super::next_action::summary as next_action_summary;
use super::*;
...
fn get_unchecked_mut_bounds_evidence_requires_same_receiver() -> Result<(), String> {
    ...
    let reassigned_receiver = fixture_output("get_unchecked_mut_reassigned_receiver_not_guard")?;
    ...
    let other_slice_probe = fixture_output("get_unchecked_mut_get_probe_other_slice_not_guard")?;
...
fn box_from_raw_uses_ownership_operation_family() -> Result<(), String> {
    ...
    assert!(
        card.obligation_evidence
            .iter()
            .any(|evidence| evidence.obligation.key == "ownership" && !evidence.discharge.present),
...
fn copy_range_evidence_accepts_slice_length_early_returns() -> Result<(), String> {
    let copy_nonoverlapping = fixture_output("copy_nonoverlapping_slice_range_early_return_guard")?;
    let copy_nonoverlapping_card = single_card(
...
fn copy_range_evidence_accepts_open_slice_length_branches() -> Result<(), String> {
    let copy_nonoverlapping = fixture_output("copy_nonoverlapping_slice_range_open_branch_guard")?;
    let copy_nonoverlapping_card = single_card(
...
fn suppression_policy_marks_exact_card_identity_as_suppressed() -> Result<(), String> {
    let root = copy_fixture_to_temp("raw_pointer_alignment", "unsafe-review-suppression-match")?;
    let card_id = single_card("raw_pointer_alignment", &fixture_output_at(&root)?)?
        .id
        .0
        .clone();
...
fn configured_receipt_keeps_witness_gap_visible() -> Result<(), String> {
    let root = copy_fixture_to_temp("raw_pointer_alignment", "unsafe-review-configured-receipt")?;
    let card_id = single_card("raw_pointer_alignment", &fixture_output_at(&root)?)?
        .id
        .0
        .clone();
...
fn wrong_tool_receipt_keeps_witness_gap_visible() -> Result<(), String> {
    let root = copy_fixture_to_temp("raw_pointer_alignment", "unsafe-review-wrong-tool-receipt")?;
    let card_id = single_card("raw_pointer_alignment", &fixture_output_at(&root)?)?
        .id
        .0
        .clone();
...
fn write_receipt_with_strength(root: &Path, card_id: &str, strength: &str) -> Result<(), String> {
