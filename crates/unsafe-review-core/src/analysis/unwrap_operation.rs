...
pub(super) fn unwrap_operation_family(line: &str) -> Option<OperationFamily> {
    contains_call_name(line, "unwrap_unchecked").then_some(OperationFamily::UnwrapUnchecked)
}
...
mod tests {
    ...
    #[test]
    fn detects_unwrap_unchecked_calls() {
        assert_eq!(
            unwrap_operation_family("unsafe { option.unwrap_unchecked() }"),
            Some(OperationFamily::UnwrapUnchecked)
        );
    }
    ...
    fn ignores_identifier_suffix_matches() {
        assert_eq!(
            unwrap_operation_family("unsafe { option.not_unwrap_unchecked() }"),
            None
        );
    }
}
