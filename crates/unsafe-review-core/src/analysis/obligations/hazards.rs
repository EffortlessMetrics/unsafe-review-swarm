...
pub(super) fn hazards_for(family: &OperationFamily) -> Vec<HazardKind> {
    match family {
        OperationFamily::RawPointerDeref
        | OperationFamily::RawPointerRead
        | OperationFamily::RawPointerWrite => raw_pointer_access_hazards(),
        OperationFamily::RawPointerReadUnaligned | OperationFamily::RawPointerWriteUnaligned => {
            unaligned_raw_pointer_access_hazards()
        }
...
fn raw_pointer_access_hazards() -> Vec<HazardKind> {
    vec![
        HazardKind::PointerValidity,
        HazardKind::Alignment,
        HazardKind::InitializedMemory,
...
fn unaligned_raw_pointer_access_hazards() -> Vec<HazardKind> {
    raw_pointer_access_hazards()
        .into_iter()
        .filter(|hazard| *hazard != HazardKind::Alignment)
        .collect()
}
