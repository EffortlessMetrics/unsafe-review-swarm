use crate::domain::{
    EvidenceState, MissingEvidence, ObligationEvidence, RelatedTest, ReviewCard, WitnessRoute,
};
...
pub(super) fn required_safety_conditions(card: &ReviewCard) -> Vec<&str> {
    card.obligations
        .iter()
...
impl<'a> From<&'a ReviewCard> for AgentSafetyContract<'a> {
    fn from(card: &'a ReviewCard) -> Self {
        Self {
            required_conditions: required_safety_conditions(card),
...
impl<'a> From<&'a MissingEvidence> for AgentMissingEvidence<'a> {
    fn from(missing: &'a MissingEvidence) -> Self {
        Self {
            kind: &missing.kind,
