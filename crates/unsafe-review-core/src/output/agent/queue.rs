...
pub(super) fn agent_readiness(card: &ReviewCard, has_card_scoped_repairs: bool) -> AgentReadiness {
    readiness::build(card, has_card_scoped_repairs)
}

pub(super) fn allowed_repairs(card: &ReviewCard) -> AllowedRepairs {
    repairs::build(card)
}

pub(super) fn repair_queue(card: &ReviewCard, readiness: &AgentReadiness) -> AgentRepairQueue {
