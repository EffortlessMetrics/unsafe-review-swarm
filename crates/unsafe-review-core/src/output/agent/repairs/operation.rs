...
pub(super) fn add_operation_repairs(card: &ReviewCard, repairs: &mut Vec<String>) {
    match card.operation.family {
        ...
        OperationFamily::StableByteSourceNativeFfiRead
            if common::missing_discharge(card, "byte-stability") =>
        {
            repairs.push("reject overlapping JS-backed input/output byte spans or copy input bytes into stable disjoint storage before native FFI handoff".to_string());
