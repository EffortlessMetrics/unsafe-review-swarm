...
<!-- Public Shields endpoint JSON under badges/ is generated by `cargo xtask badges`; verify with `cargo xtask badges --check`. `unsafe-review` badges are repo-scoped static review evidence, not memory-safety proof, not UB-free status, and not Miri-clean status. -->
