#![forbid(unsafe_code)]
use source_sync::{parse_rev_list_counts, report_source_divergence, source_sync_checkpoint};
    ...
    match args.get(1).map(|arg| arg.as_str()) {
        ...
        Some("source-divergence") | Some("check-source-sync") => {
            ...
            let checkpoint = source_sync_checkpoint(
                ...
                parse_toml_file,
                require_toml_string,
                |value, key, path| required_toml_string(value, key, path).map(str::to_string),
                require_file,
            )?;
            report_source_divergence(checkpoint)
        }
