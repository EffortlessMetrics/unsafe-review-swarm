...
pub(crate) const SOURCE_MAIN_REF: &str = "refs/unsafe-review-sync/source-main";
pub(crate) const SWARM_MAIN_REF: &str = "refs/unsafe-review-sync/swarm-main";

pub(crate) struct SourceSyncCheckpoint {
    pub(crate) source_repo: String,
    pub(crate) swarm_repo: String,
    pub(crate) acknowledged_source_main: String,
    pub(crate) acknowledged_by: String,
}

pub(crate) fn report_source_divergence(checkpoint: SourceSyncCheckpoint) -> Result<(), String> {
...
pub(crate) fn source_sync_checkpoint(
    source_sync_ledger: &str,
    parse_toml_file: impl FnOnce(&Path) -> Result<toml::Value, String>,
    require_toml_string: impl Fn(&toml::Value, &str, &str) -> Result<(), String>,
    required_toml_string: impl Fn(&toml::Value, &str, &str) -> Result<String, String>,
    require_file: impl Fn(&str) -> Result<(), String>,
) -> Result<SourceSyncCheckpoint, String> {
    ...
    let source_repo = required_toml_string(&value, "source_repo", source_sync_ledger)?;
    let swarm_repo = required_toml_string(&value, "swarm_repo", source_sync_ledger)?;
    let acknowledged_source_main =
        required_toml_string(&value, "acknowledged_source_main", source_sync_ledger)?;
    let acknowledged_by = required_toml_string(&value, "acknowledged_by", source_sync_ledger)?;
