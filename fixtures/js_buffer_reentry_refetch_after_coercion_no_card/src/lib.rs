pub struct JSValue;
pub struct GlobalObject;
pub struct ArrayBuffer {
    bytes: Vec<u8>,
}

impl JSValue {
    pub fn as_array_buffer(&self, _global: &mut GlobalObject) -> Result<ArrayBuffer, ()> {
        Ok(ArrayBuffer { bytes: vec![] })
    }

    pub fn coerce_to_int64(&self, _global: &mut GlobalObject) -> Result<i64, ()> {
        Ok(0)
    }
}

impl ArrayBuffer {
    pub fn byte_slice(&self) -> &[u8] {
        self.bytes.as_slice()
    }
}

pub fn index_of_line(
    global: &mut GlobalObject,
    source: JSValue,
    offset: JSValue,
) -> Result<usize, ()> {
    let _stale = source.as_array_buffer(global)?;
    let start = offset.coerce_to_int64(global)?;
    let refreshed = source.as_array_buffer(global)?;
    let bytes = refreshed.byte_slice();
    Ok(bytes.len().saturating_sub(start as usize))
}
