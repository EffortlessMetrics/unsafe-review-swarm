...
pub fn pick_after_stale_match_get_path<'a>(
    mut bag: Bag<'a>,
    index: usize,
    fallback: &'a mut [u8],
) -> Option<&'a mut u8> {
...
#[cfg(test)]
mod tests {
    use super::{pick_after_stale_match_get_path, Bag};
    ...
    fn writes_fallback_after_receiver_path_change() {
        ...
        let mut fallback = [5_u8, 6, 7, 8];
        {
            ...
            let slot = pick_after_stale_match_get_path(bag, 2, &mut fallback).unwrap();
            *slot = 9;
        }
