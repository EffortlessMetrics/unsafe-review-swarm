pub fn copy_overlapping_checked(src: &[u8], dst: &mut [u8], count: usize) {
    if src.len() >= count {
        let _observed_source_range = true;
    }
    if dst.len() >= count {
        let _observed_destination_range = true;
    }
    // SAFETY: fixture checks that closed branch range observations are not ptr::copy range evidence.
