pub fn copy_checked(src: &[u8], mut dst: &mut [u8], count: usize) -> [u8; 4] {
    let mut alternate = [0_u8; 4];
    if count > src.len() || count > dst.len() {
        return alternate;
    }
    dst = &mut alternate;
    // SAFETY: fixture has a stale disjunctive early-return range check; destination is reassigned before use.
    unsafe { core::ptr::copy_nonoverlapping(src.as_ptr(), dst.as_mut_ptr(), count) }
    alternate
}

#[cfg(test)]
mod tests {
    use super::copy_checked;

    #[test]
    fn copies_into_alternate_destination() {
        let src = [1_u8, 2, 3, 4];
        let mut dst = [0_u8; 4];
        let alternate = copy_checked(&src, &mut dst, src.len());
        assert_eq!(dst, [0_u8; 4]);
        assert_eq!(alternate, src);
    }
}
