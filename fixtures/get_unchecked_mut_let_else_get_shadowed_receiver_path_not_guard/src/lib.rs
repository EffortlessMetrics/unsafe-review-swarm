...
pub fn pick_after_shadowed_let_else_get_path<'a>(
    bag: Bag<'a>,
    index: usize,
    fallback: &'a mut [u8],
) -> Option<&'a mut u8> {
    let Some(_) = bag.values.get(index) else {
        return None;
    };
...
#[cfg(test)]
mod tests {
    use super::{pick_after_shadowed_let_else_get_path, Bag};
    ...
    fn writes_fallback_after_receiver_path_shadowing() {
        ...
        let mut fallback = [5_u8, 6, 7, 8];
        {
            ...
            let slot = pick_after_shadowed_let_else_get_path(bag, 2, &mut fallback).unwrap();
            *slot = 9;
        }
