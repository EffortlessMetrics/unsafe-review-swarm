pub fn pick_after_stale_get_probe_return_receiver<'a>(
    mut values: &'a mut [u8],
    index: usize,
    fallback: &'a mut [u8],
) -> Option<&'a mut u8> {
...
#[cfg(test)]
mod tests {
    use super::pick_after_stale_get_probe_return_receiver;

    #[test]
    fn writes_fallback_after_receiver_change() {
        let mut values = [1_u8, 2, 3, 4];
        let mut fallback = [5_u8, 6, 7, 8];
        let slot = pick_after_stale_get_probe_return_receiver(&mut values, 2, &mut fallback)
