...
| `bun-stable-byte-node-fs-rab-scalar-write` | `fixtures/js_buffer_reentry_node_fs_rab_scalar_write` | `fixtures/js_buffer_reentry_node_fs_rab_scalar_write_scheduled_before_capture_no_card` | `Stable-byte RAB async heuristic` | Confirms node:fs scalar write input capture before async worker dispatch and worker-side materialization static shape only, while scheduling before capture stays no-card. Encoded writeFile/appendFile fixture pressure remains separate calibration/support-tier context, not this seed's first PR aperture. `observable-red-green` proof still needs external system Bun red and patched-green evidence. |
