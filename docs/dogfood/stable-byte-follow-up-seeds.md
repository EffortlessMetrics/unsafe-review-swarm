...
| `bun-stable-byte-node-fs-rab-scalar-write` | `fixtures/js_buffer_reentry_node_fs_rab_encoded_write_file` | `fixtures/js_buffer_reentry_node_fs_rab_encoded_write_scheduled_before_capture_no_card` | `Stable-byte RAB async heuristic` | Confirms encoding-aware node:fs writeFile/appendFile input capture before async worker dispatch and worker-side materialization static shape only; scalar write remains covered by `fixtures/js_buffer_reentry_node_fs_rab_scalar_write` in calibration/support-tier ledgers. `observable-red-green` proof still needs external system Bun red and patched-green evidence. |
