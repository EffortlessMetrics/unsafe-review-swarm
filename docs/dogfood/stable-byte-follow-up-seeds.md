...
| `bun-stable-byte-zstd-overlap-native-ffi` | `fixtures/stable_byte_zstd_native_ffi_read` | `fixtures/stable_byte_zstd_disjoint_boundary_no_card` | `Stable-byte native FFI read heuristic` | Confirms JS-backed input and mutable output byte spans reach native FFI handoff before disjointness or copy-boundary evidence in the static fixture only; `observable-red-green` proof still needs external system Bun red and patched-green evidence. |
