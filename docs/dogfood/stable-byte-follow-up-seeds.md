...
| `bun-stable-byte-node-fs-rab-scalar-write` | `fixtures/js_buffer_reentry_node_fs_rab_encoded_write_file` | `fixtures/js_buffer_reentry_node_fs_rab_encoded_write_scheduled_before_capture_no_card`, `fixtures/js_buffer_reentry_node_fs_rab_encoded_write_recapture_after_dispatch_no_card` | `Stable-byte RAB async heuristic` | Confirms encoding-aware node:fs writeFile/appendFile input capture before async worker dispatch and worker-side materialization static shape only, while a fresh encoded input container recaptured after dispatch stays no-card. Scalar write remains covered by `fixtures/js_buffer_reentry_node_fs_rab_scalar_write` in calibration/support-tier ledgers. `observable-red-green` proof still needs external system Bun red and patched-green evidence. |
...
| `bun-stable-byte-zstd-overlap-native-ffi` | `fixtures/stable_byte_native_ffi_zstd_handoff` | `fixtures/stable_byte_native_ffi_zstd_owned_copy_control`, `rtk cargo run --locked -p xtask -- check-manual-candidate-examples` | `Stable-byte native FFI heuristic / manual-candidate smoke` | Confirms JS-backed byte capture before zstd-style native pointer/length FFI handoff emits an advisory stable-byte card while retaining the generic FFI seam; the owned-copy control stays FFI-only. No overlap detection is claimed, no site execution is claimed, and `observable-red-green` proof still needs external system Bun red and patched-green evidence. |
