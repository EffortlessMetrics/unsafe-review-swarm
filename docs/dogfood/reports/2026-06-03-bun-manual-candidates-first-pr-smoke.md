...
| `review-kit.json` | Verified | Includes a bounded manual-candidate queue plus proof-mode, stable-byte-class, ledger-state, oracle-map, and guidance mix counts that match `manual-candidates.json`; candidates stay out of the ReviewCard repair queue. |
