...
| `tokmd-packets.json` | Verified | Preserves manual IDs, proof mode, fix boundary, PR aperture, implementer handoff, copy-only commands, joined stable-byte seed rows, and ReviewCard-only comment-plan review-budget metadata as formatting input only; tokmd is not run and missing ledger/receipt inputs stay explicit. |
