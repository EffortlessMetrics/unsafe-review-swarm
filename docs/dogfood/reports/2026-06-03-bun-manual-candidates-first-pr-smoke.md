...
| `tokmd-packets.json` | Verified | Preserves manual IDs, proof mode, fix boundary, PR aperture, implementer handoff, copy-only commands, and joined stable-byte seed rows as formatting input only; tokmd is not run and missing ledger/receipt inputs stay explicit. |
| `review-kit.json` | Verified | Includes a bounded manual-candidate queue plus proof-mode, stable-byte-class, ledger-state, oracle-map, guidance mix counts, and joined stable-byte seed owner/next-PR metadata that match `manual-candidates.json`; candidates stay out of the ReviewCard repair queue. |
