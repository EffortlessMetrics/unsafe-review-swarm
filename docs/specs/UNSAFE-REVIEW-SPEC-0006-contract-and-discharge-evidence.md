...
| Box raw ownership evidence | same-pointer `Box::into_raw` origin before `Box::from_raw` or `ptr::drop_in_place` | origin is for another pointer, appears only after the unsafe operation, or the raw pointer is reassigned before use | `box_from_raw_box_origin`, `box_from_raw_reassigned_origin_not_guard`, `box_from_raw_box_origin_after_not_guard`, `drop_in_place_box_origin`, `drop_in_place_reassigned_origin_not_guard`, `drop_in_place_box_origin_after_not_guard` |
