...
|---|---|---|---|
| Unsafe API contract evidence | public unsafe declarations have `# Safety` docs or doc-comment `Safety:` prose; private unsafe declarations and local unsafe call wrappers may use nearby `# Safety`, `SAFETY:`, or `Safety:` prose as contract evidence only | local `SAFETY:` comments do not satisfy public API docs, and contract evidence does not erase missing guard/discharge evidence | `public_unsafe_fn_missing_safety`, `public_unsafe_trait_missing_safety`, `split_public_unsafe_fn_missing_safety`, `public_unsafe_fn_with_safety_docs`, `public_unsafe_fn_safety_colon_docs`, `public_unsafe_fn_safety_comment_not_docs`, `documented_private_unsafe_fn`, `private_unsafe_helper_safety_comment`, `local_safety_colon_comment` |
...
| Box raw ownership evidence | same-pointer `Box::into_raw` origin before `Box::from_raw` or `ptr::drop_in_place` | origin is for another pointer, appears only after the unsafe operation, or the raw pointer is reassigned before use | `box_from_raw_box_origin`, `box_from_raw_reassigned_origin_not_guard`, `box_from_raw_box_origin_after_not_guard`, `box_from_raw_other_origin_not_guard`, `drop_in_place_box_origin`, `drop_in_place_reassigned_origin_not_guard`, `drop_in_place_box_origin_after_not_guard`, `drop_in_place_other_origin_not_guard` |
