...
| `UNSAFE-REVIEW-SPEC-0024` CI design | draft | CI lane taxonomy and permissions posture documented; no new workflow behavior in the draft spec PR | `cargo run --locked -p xtask -- check-ci-lanes`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-21 | CI hard-fails malformed artifacts and repo policy failures, not advisory unsafe-review findings by default |
...
| `UNSAFE-REVIEW-SPEC-0026` accuracy validation and calibration | proposed | Label-ledger checker landed with fixture-pinned evidence and route-quality ledgers for raw pointer alignment, public unsafe API docs, Vec::set_len, transmute bool validity, unsafe impl Send/Sync, and FFI | `cargo run --locked -p xtask -- check-calibration`; `cargo run --locked -p xtask -- check-dogfood`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-23 | Claim-scoped calibration only; no global precision/recall, no policy-ready claims |
