...
| `UNSAFE-REVIEW-SPEC-0026` accuracy validation and calibration | proposed | Label-ledger checker landed with fixture-pinned evidence, route-quality, and no-card honesty ledgers for raw pointer alignment, public unsafe API docs, MaybeUninit::assume_init, Vec::set_len, transmute bool validity, unsafe impl Send/Sync, FFI, and zero-card controls | `cargo run --locked -p xtask -- check-calibration`; `cargo run --locked -p xtask -- check-dogfood`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-23 | Claim-scoped calibration only; no global precision/recall, no policy-ready claims |
