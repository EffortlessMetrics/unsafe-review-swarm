...
| `UNSAFE-REVIEW-SPEC-0026` accuracy validation and calibration | proposed | Rail scaffold added: spec + lane tracker + initial policy ledger/docs; checker wiring remains follow-up | `cargo run --locked -p xtask -- check-doc-artifacts`; `cargo run --locked -p xtask -- check-goals`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-21 | Claim-scoped calibration only; no global precision/recall, no policy-ready claims |
