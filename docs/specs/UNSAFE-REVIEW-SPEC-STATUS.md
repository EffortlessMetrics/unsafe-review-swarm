...
| `UNSAFE-REVIEW-SPEC-0026` accuracy validation and calibration | proposed | Label-ledger checker landed with fixture-pinned evidence, route-quality, and no-card honesty ledgers for raw pointer alignment/write initialization, public unsafe API docs, Box::from_raw ownership, ptr::drop_in_place Box-origin evidence/routes, generic unsafe call callee-contract evidence, get_unchecked_mut bounds evidence, copy_nonoverlapping valid-range, ptr::copy valid-range, NonNull::new_unchecked nullability, MaybeUninit::assume_init, Vec::set_len, Vec::from_raw_parts capacity, transmute bool validity, UTF-8 unchecked conversion, unsafe impl Send/Sync, FFI route/obligation evidence, and zero-card controls | `cargo run --locked -p xtask -- check-calibration`; `cargo run --locked -p xtask -- check-dogfood`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-23 | Claim-scoped calibration only; no global precision/recall, no policy-ready claims |
