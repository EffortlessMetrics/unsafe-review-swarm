...
| `UNSAFE-REVIEW-SPEC-0026` accuracy validation and calibration | proposed | Label-ledger checker landed with fixture-pinned raw pointer alignment and public unsafe API contract-evidence ledgers | `cargo run --locked -p xtask -- check-calibration`; `cargo run --locked -p xtask -- check-dogfood`; `cargo run --locked -p xtask -- check-pr`; `cargo run --locked -p xtask -- source-divergence` | 2026-05-23 | Claim-scoped calibration only; no global precision/recall, no policy-ready claims |
