...
- `owner`
- `reason`
- `evidence`: a typed evidence value starting with a recognized prefix (see
  taxonomy below) followed by at least one non-whitespace character
...
The `evidence` field must use one of the following typed prefixes, aligned with
the cargo-allow interop contract (`docs/interop/sibling-tools.md`):

| Prefix | Meaning |
|---|---|
| `test:` | test name or test file path |
| `doc:` | documentation link or reference |
| `spec:` | spec identifier (e.g. `spec: SPEC-0010`) |
| `adr:` | ADR identifier (e.g. `adr: ADR-0001`) |
| `ripr:` | ripr mutation-test reference |
| `unsafe-review:` | unsafe-review card or receipt reference |
| `coverage:` | coverage measurement or artifact |
| `issue:` | issue number or link |
| `pr:` | pull request number or link |
| `baseline-init:` | generated by `baseline init`; pre-existing debt capture |

Untyped values like `"manual review"` are rejected by `check-policy`.
The `baseline init` command generates `baseline-init:` evidence automatically.
...
- Non-empty baseline entries require exact counted identity, owner, reason,
  typed evidence, and `review_after`.
- Non-empty suppression entries require exact counted identity, owner, reason,
  typed evidence, and either `review_after` or `expires`.
- Uncounted card identities are rejected.
- Untyped evidence values (e.g. `"manual review"`) are rejected by `check-policy`.
- Typed evidence values (e.g. `"issue: #1523"`) are accepted by `check-policy`.
