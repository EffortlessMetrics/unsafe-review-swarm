...
authorization, or source-repository decision. Snapshot date: 2026-08-10.

## Repository snapshot

| Repository | Base | Source-divergence posture |
| --- | --- | --- |
| `EffortlessMetrics/unsafe-review-swarm` | `9285040513b80279b3f2570e3ab4ede37b3ffb27` | `new_source_commits=0`; expected unpromoted swarm work remains |
...
This refresh records the swarm-side resolution of #2014 and makes no other
dependency or workflow change. The #2014 repair followed the recorded paired
path — workflow pins and allowlist entries updated in the same PR, then policy
and Rust proof rerun — rather than weakening the allowlist or bypassing the
deterministic gate. The source Actions PR #549 has the same policy shape and
remains owner-gated.
