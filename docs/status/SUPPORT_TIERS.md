...
| Repo inventory, posture Markdown, and badge JSON | experimental | `repo --format json`, `repo --format markdown`, `badges` | core Markdown and serde-backed badge renderer tests plus CLI e2e cover repo-scope open-gap counts, posture sections, per-card source location, operation expression, operation family, next actions, trust boundary, and badge messages for fixture card and zero-gap cases | static open-gap counts only; not calibrated, not a safety badge, and not policy gating |
