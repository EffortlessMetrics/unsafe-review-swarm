...
| Witness receipt audit | experimental | `receipt audit` | core receipt-audit tests cover matched, importable-as-witness, unmatched, stale, expired, wrong-identity, wrong-tool, weaker-than-required, command-hash-mismatch, duplicate, invalid metadata, saved summary/author/recorded/command-hash/limitation fields, and matched-card operation expression, operation family, and next-action context; CLI e2e covers JSON and Markdown audit metadata projection | advisory metadata audit only; does not execute witnesses, infer site reach, make policy decisions, or prove witness success |
