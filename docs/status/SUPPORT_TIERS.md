...
| Repo inventory, posture Markdown, status sidecars, and badge JSON | experimental | `repo --format json`, `repo --format markdown`, `repo --out <file> --progress`, `badges` | core Markdown and serde-backed badge renderer tests plus CLI e2e cover repo-scope open-gap counts, posture sections, per-card source location, operation expression, operation family, next actions, trust boundary, `<out>.status.json` completion status, progress heartbeat, and badge messages for fixture card and zero-gap cases | static open-gap counts and operational scan status only; not calibrated, not a safety badge, not policy gating, and not partial-report or interruption preservation |
