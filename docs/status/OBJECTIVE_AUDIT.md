...
That run exposed unsafe function pointer fields with empty owner identity; the
`unsafe_fn_pointer_field_owner` fixture now pins field-name owner identity while
leaving those cards as advisory `unsafe_fn` / `unknown` contract cards.
