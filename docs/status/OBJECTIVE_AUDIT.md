...
fixture-pinned claims, 449 calibration cases, 34 label ledgers, and 462 label
...
`assume_init_ref`, and `assume_init_mut`, open-branch `write`, and
`MaybeUninit::new` initialization evidence while rejecting other-slot writes for
`assume_init` and `assume_init_read`, closed conditional writes, and stale
