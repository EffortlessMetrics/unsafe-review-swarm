...
fixture-pinned claims, 247 calibration cases, 34 label ledgers, and 258 label
...
- `str::from_utf8_unchecked` now recognizes same-buffer `is_ok`, if-let `Ok`,
  `is_err` early-return, question-mark propagation, match-return validation, and
  match `Ok` arm validation evidence, and has post-validation, wrong-buffer, and
