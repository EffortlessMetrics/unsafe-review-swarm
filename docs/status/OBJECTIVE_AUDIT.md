...
fixture-pinned claims, 479 calibration cases, 34 label ledgers, and 492 label
...
`write`, same-slot `MaybeUninit::new` initialization evidence for
`assume_init`, `assume_init_read`, `assume_init_ref`, and `assume_init_mut`, and open-branch `MaybeUninit::new`
evidence for `assume_init` while rejecting other-slot writes for `assume_init`,
