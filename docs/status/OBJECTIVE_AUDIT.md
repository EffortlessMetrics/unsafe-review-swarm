...
| Copy range evidence stays operation-specific | `copy_nonoverlapping_other_len_not_guard` and `ptr_copy_other_len_not_guard` prove unrelated slice length assertions do not discharge copy source/destination range obligations | Experimental | Positive copy range evidence beyond operation smoke remains limited |
