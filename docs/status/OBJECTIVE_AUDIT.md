...
controls that reject other-index checks, bare predicate observations, and
post-use checks.
...
- Real PR-diff dogfood now recognizes `index < self.num_ctrl_bytes()` as bounds
  ...
  rejects other-index guards, bare observations, and post-use checks. Capped
  `memchr` repo dogfood recognizes the local same-slice `as_ptr()` plus `len()`
  end-pointer pattern, but broader pointer-arithmetic guard naming remains
  uncalibrated.
