...
`get(index)` probe guards plus false-positive controls for bare predicate
observations, closed positive branches, comment-only early-return text, and
checked indexes reassigned before the unchecked access.
...
fixture-pinned claims, 233 calibration cases, 34 label ledgers, and 244 label
...
narrow same-slot `write` and `MaybeUninit::new` initialization evidence while
rejecting stale writes after reassignment.
...
- `str::from_utf8_unchecked` now recognizes same-buffer `is_ok`, if-let `Ok`,
  `is_err` early-return, question-mark propagation, and match-return validation
  evidence, and has post-validation and wrong-buffer false-positive-control
