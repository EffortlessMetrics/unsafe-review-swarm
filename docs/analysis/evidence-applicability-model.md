...
| `MaybeUninit::assume_init` | `MaybeUninitSlotContext` for same-slot writes/new bindings, scope reach, and stale slot checks | factored | Partial-field initialization is fixture-pinned as non-discharge evidence; add array or broader field-pattern recognition only as separate fixture-backed slices. |
