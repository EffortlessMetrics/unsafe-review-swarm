...
| `NonNull::new_unchecked` | Same pointer | Nullability guard applies to the pointer passed to unchecked constructor. | Wrong pointer, stale or shadowed pointer/receiver, non-returning `is_null`, cast-target/provenance uncertainty. |
...
| `NonNull::new_unchecked` | `NonNullPointerContext` for same-pointer probes, top-level conjunctive non-null open branches, top-level disjunctive null early-return branches, stale or shadowed pointer/receiver checks, and cast-target uncertainty; inverse branch shapes remain non-discharge controls | factored | Add macro controls only from concrete fixtures. |
...
| `transmute` / `transmute_copy` | `TransmuteLayoutContext` and `TransmuteValueDomainContext` separate layout-size evidence from value-domain evidence; layout checks accept top-level conjunctive open branches and mismatch early returns while rejecting disjunctive open branches, conjunctive mismatch returns, observed equality, and closed-branch assertions, and the value-domain context owns same-source-value and stale-reassignment checks | factored | Do not broaden valid-value domains without one positive and one false-positive control. |
