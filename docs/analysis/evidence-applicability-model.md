...
| `transmute` / `zeroed` | Same source/destination type and value | Layout evidence must dominate the operation; valid-value/valid-zero evidence is type-specific and must keep the source value fresh after the guard. | Layout-only prose, closed-branch layout assertions, comment-only value claim, stale byte/value check. |
...
| `transmute` / `transmute_copy` | `TransmuteLayoutContext` and `TransmuteValueDomainContext` separate layout-size evidence from value-domain evidence; layout checks reject closed-branch assertions, and the value-domain context owns same-source-value and stale-reassignment checks | factored | Do not broaden valid-value domains without one positive and one false-positive control. |
