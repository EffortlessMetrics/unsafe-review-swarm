...
| `Vec::set_len` | Same receiver and initialized range | Initialization/capacity/shrink evidence applies to the new length. | Capacity-only evidence, stale receiver, partial-slice initialization, single-index initialization, unrelated initialization, comment-only init. |
