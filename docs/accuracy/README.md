...
- `str::from_utf8_unchecked` UTF-8 validation evidence: checks whether the
  ...
  including if-let `Ok` branch validation, let-else `Ok` validation, and match
  `Ok` arm validation, while rejecting post-validation, wrong-buffer,
  observed-only, and stale-buffer controls.
