...
- `Vec::set_len` initialized-range evidence: checks whether the `initialized`
  ...
  while rejecting capacity-only, comment-only, unrelated-capacity,
  cap-name-only, stale receiver, post-init, partial-slice initialization,
  single-index initialization, and unrelated initialization controls.
...
- `MaybeUninit::assume_init` family initialized-memory evidence: checks that
  ...
  `SAFETY` prose, stale writes, partial-field writes, and partial-array writes
  remain missing discharge evidence, and `assume_init`, `assume_init_read`,
...
- `transmute::<u8, bool>` / `transmute_copy::<u8, bool>` valid-value evidence:
  ...
  comment-only, observed, closed-branch, stale-byte, and multiline
  `transmute_copy` controls.
