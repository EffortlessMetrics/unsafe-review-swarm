...
| 4a | check: treat valid empty diffs as diff-scoped no-op runs | #1558 | malformed diff = input error (already shipped, #1557); valid empty diff = complete no-op (0 files, 0 cards, no-new-debt passes, scope stays diff); nonempty diff = selected files only; no whole-repo cards under scope=diff |
| 5 | repo: status artifact when --max-cards stops analysis | #1545 | ✅ Shipped: a capped scan emits `partial: true`, `stop_reason: "max_cards"`, `cap: N`, `cards_found: N`, and a cap-specific `operator.next_action`; timeout emits `stop_reason: "timeout"`, terminated emits `stop_reason: "terminated"`; complete scans emit `partial: false`, `stop_reason: "none"`. SPEC-0035 field names reconciled to shipped reality (`scan_scope`, `elapsed_ms`, `files_discovered/files_scanned/files_remaining`, `cards_found`, `completed`, `partial`, `stop_reason`, `cap`, `error`, `signal`, `partial_path`, `operator`); phase vocabulary corrected (`complete/failed/terminated`, not `done/timed_out`); timeout stays `phase: failed` + `stop_reason: timeout` (not a new `timed_out` phase — minimal change). Gate manifest not produced by capped repo path; no manifest change needed. |
| 6 | docs: scan cost and large-repo scoping | #1546 | ✅ Shipped: CLI guide "Scan cost and large-repo scoping" section — per-file cost expectation (size/density-driven), large repos intractable unscoped, deliberate scoping workflow (include/exclude → list-files preview → timeout/max-cards bound → re-run narrower), partial-scan-is-a-snapshot framing matching the #1545 stop_reason behavior; no completeness overclaims |
